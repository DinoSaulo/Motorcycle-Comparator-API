...
const JSON_PATH = path.join(SOURCE_DIR, 'royal_enfield_motos.json');
const SQL_PATH = path.join(REPO_ROOT, 'src/main/resources/db/seed/R__motorcycles_royal_enfield_specs_2026_08.sql');
...
function clean(text, maxLength) {
    ...
    // The "fl" ligature was lost on the Brazilian brake sheet, leaving "¬utuante" for "flutuante".
    let t = before.replace(/¬utuante/g, 'flutuante').replace(/�/g, ' ');
...
function cylindersOf(engineText, explicit) {
    ...
    // Adjacency to "cylinder"/"cilindro" is required: "Inline Twin cylinder, 4 stroke" describes a
    // twin, and a bare leading "Four Stroke" must not be read as a cylinder count.
    ...
    // The Brazilian sheet spells the count out rather than writing a digit:
    // "REFRIGERAÇÃO LÍQUIDA, UM CILINDRO, DOHC, 4 VÁLVULAS".
    ...
    const tri = /\btricil\w*/i.exec(engineText);
...
function valvesPerCylinderOf(engineText, cylinders) {
    ...
    // A bare total ("SOHC, 4 válvulas") only becomes a per-cylinder figure once the count is known.
...
function compressionOf(text) {
    if (!text) return null;
    // "9.5:1", "9,5 : 1" and "9.5.1" all describe the same thing; store one shape.
...
const CATEGORY_BY_SOURCE_LABEL = {
    ...
    adventure: 'ADVENTURE', scrambler: 'OFF_ROAD', touring: 'TOURING',
};
...
for (const moto of snapshot.motos) {
    ...
    rows.push({
        ...
        kerb_weight_kg: kerb,
        dry_weight_kg: dry,
        // Litres only. "14.5 Litres / 3.8 US gal" must never fall through to the gallon figure.
...
w('-- Motorcycle Comparison API - Royal Enfield technical specifications');
w(`-- Generated from zontes-scraper/royal_enfield_motos.json (scraped ${snapshot.gerado_em}) by tools/import-royal-enfield-specs.mjs.`);
w('--');
...
w('-- (149 from the FIPE snapshot, 3 from R__dev_seed.sql), so against today\'s seeds this import creates no');
w(`-- motorcycles and only fills gaps. ${emitted.length === snapshot.motos.length ? 'All of them carry' : `${emitted.length} of them carry`} something usable and are emitted. The insert`);
w('-- path below still runs: whether a row is new is derived from the database at run time rather than');
w('-- from a flag baked in here, so a slug the catalogue does not have is created rather than dropped.');
w('--');
...
w('-- form only (the sheets print "14.5 Litres / 3.8 US gal", and the gallon figure must never win). The');
w('-- sources disagree on decimal convention inside the same file ("15,7 litros" beside "13.5 Litres"), so');
w('-- a separator is read from its own shape: exactly three trailing digits is a thousands group, anything');
w('-- else is a decimal point.');
w('--');
...
w(`-- disagrees, and here it disagrees on ${CORRECTED.count} rows. The 648 cc engine (78 x 67.8 mm, exactly two`);
w('-- cylinders\' worth) is captioned "Single cylinder", and the 349 cc engine (72 x 85.8 mm, exactly one)');
w('-- is captioned "Parallel Twin" - the two captions are swapped at source. The arithmetic lands within');
w('-- 0.1% of a whole number in both cases, so it wins, and engine_type is relabelled to match rather');
w('-- than left contradicting the column beside it. Everything else in the description is untouched.');
w('--');
w('-- An adjustable seat is published as a range ("825 - 845 mm"). The low end is stored: that is the');
w('-- height the bike stands at as delivered, and the high end would overstate it for every rider who');
w('-- never raises the seat. Ranges are collapsed for measurements only, never for power or torque.');
w('--');
w(`-- Two encoding faults in the snapshot are repaired on the way in (${REPAIRED.count} values). The Brazilian brake sheet`);
w('-- lost its "fl" ligature and reads "¬utuante" where the manufacturer wrote "flutuante", and one power');
w('-- string carries a replacement character where its sheet had a space. Both are extraction damage, not');
w('-- source content. A third fault is left alone: the extractor also spaces digits out ("curso 20 0 mm"');
w('-- for a 200 mm shock), and a number it has broken up is refused rather than read as its first');
w('-- fragment, because a plausibility bound cannot catch a wrong figure that lands inside the range.');
w(`-- ${MALFORMED.count} numeric reads were refused on that rule; the garbled strings this snapshot carries all sit in`);
w('-- suspension text, which is stored as the source wrote it and never parsed for a figure.');
w('--');
...
w('--');
w('-- Caveat worth knowing before trusting any single row: the official Royal Enfield sheet covers only the');
w('-- line in production, so everything out of production leans on motorcyclespecs.co.za, which publishes');
w('-- the global sheet rather than the Brazilian one and reuses a single sheet across a model\'s whole');
w('-- production run. Figures are representative of the model, not specific to the year on the row. The');
w('-- emission standard is stored as the manufacturer published it: Royal Enfield builds for India first');
w('-- and quotes BS6 where a European sheet would quote Euro 5, and mapping one onto the other would');
w('-- assert an equivalence nobody published.');
w('--');
w('-- Images. The scraper stores its gallery under img/royal-enfield/<model>/, which is per model, not per');
w(`-- model-year, so the ${distinctImages} distinct photos below cover ${withImage} of the ${emitted.length} rows. Each row still gets its own`);
w('-- stored copy under a name derived from its slug (UUID v5, matching the shape FileStorageServiceImpl');
w('-- will read): MotorcycleService treats the file as owned by the row and deletes it when the motorcycle');
w('-- or its image is deleted, so a shared file name would blank every other row pointing at it. The');
w(`-- remaining ${withoutImage.length} rows are the Bullet 500 and the Classic Chrome 500 EFI, for which no source published a`);
w('-- photo at all. Those files are NOT in the repository - uploads/ is gitignored. Run');
w('-- `node tools/import-royal-enfield-specs.mjs` to materialise them from a zontes-scraper checkout;');
...
w('-- three rows here - the hand-curated Classic 350, Himalayan 450 and Guerrilla 450 Apex in');
w('-- R__dev_seed.sql - because the other 149 arrived from FIPE with every specification column NULL, so');
w('-- for them a gap-fill is a full population. Long-tail specs use ON CONFLICT DO NOTHING, which leaves');
w('-- the FIPE seed\'s own \'Fuel\' and \'Reference price (BRL)\' intact, and reuse the key names the Honda and');
w('-- Yamaha imports chose so a cross-brand comparison lines its rows up. Category is deliberately not');
w('-- touched: the source labels (Classic, Cafe Racer, Roadster, Custom) do not partition onto the');
w('-- catalogue enum, and overwriting on a lossy mapping would degrade the segment filter rather than');
w('-- improve it. Repeatable and idempotent: re-running changes nothing once it has been applied.');
w('--');
...
w(`--   dims:   kerb_weight_kg ${populated.kerb_weight_kg}, seat_height_mm ${populated.seat_height_mm}, fuel_capacity_l ${populated.fuel_capacity_l} (${withDimensions} rows get a dimension block)`);
w(`--   image_url ${withImage}; long-tail spec rows: ${specRowCount}`);
w();
w('BEGIN;');
w();
w('CREATE TEMP TABLE tmp_royal_enfield_spec_import (');
...
w(') ON COMMIT DROP;');
w();
w('CREATE TEMP TABLE tmp_royal_enfield_spec_kv (');
...
for (let start = 0; start < emitted.length; start += BATCH) {
    ...
    w(`INSERT INTO tmp_royal_enfield_spec_import (${IMPORT_COLUMNS.join(', ')}) VALUES`);
    batch.forEach((row, i) => {
...
for (let start = 0; start < kvPairs.length; start += 500) {
    ...
    w('INSERT INTO tmp_royal_enfield_spec_kv (row_no, spec_key, spec_value) VALUES');
...
w('UPDATE tmp_royal_enfield_spec_import s');
...
w('UPDATE tmp_royal_enfield_spec_import SET is_new = (motorcycle_id IS NULL);');
w();
w('DO $$');
w('BEGIN');
...
w('-- ---------------------------------------------------------------------------');
w('UPDATE tmp_royal_enfield_spec_import');
...
w('       max_torque_nm, max_torque_rpm, compression_ratio, bore_mm, stroke_mm, cooling_system, fuel_system,');
w('       transmission_type, gears, final_drive, top_speed_kph, fuel_consumption_l_100km, emission_standard');
w('FROM tmp_royal_enfield_spec_import');
w('WHERE is_new');
w('ORDER BY row_no;');
w();
...
w('FROM tmp_royal_enfield_spec_import s');
...
w('FROM tmp_royal_enfield_spec_kv k');
w('JOIN tmp_royal_enfield_spec_import s ON s.row_no = k.row_no');
...
w('    IF bad <> 0 THEN');
w("        RAISE EXCEPTION 'Royal Enfield spec import: % rows did not resolve to a catalogue row', bad;");
w('    END IF;');
w();
w('    SELECT count(*) INTO bad');
w('    FROM tmp_royal_enfield_spec_import s');
...
w('    IF bad <> 0 THEN');
w("        RAISE EXCEPTION 'Royal Enfield spec import: % rows have a mismatched engine block', bad;");
w('    END IF;');
w();
w('    SELECT count(*) INTO bad');
w('    FROM dimensions');
...
w('    IF bad <> 0 THEN');
w("        RAISE EXCEPTION 'Royal Enfield spec import: % dimension rows have dry weight above kerb weight', bad;");
w('    END IF;');
w();
...
w('    SELECT count(*) INTO bad');
w('    FROM motorcycles');
w("    WHERE lower(brand) = 'royal enfield' AND slug !~ '^[a-z0-9]+(-[a-z0-9]+)*$';");
w('    IF bad <> 0 THEN');
w("        RAISE EXCEPTION 'Royal Enfield spec import: % rows have a slug the public routing cannot use', bad;");
w('    END IF;');
w();
...
w('    IF bad <> 0 THEN');
w("        RAISE EXCEPTION 'Royal Enfield spec import: % image URLs are not a name the image endpoint can serve', bad;");
w('    END IF;');
w('END $$;');
w();
w('COMMIT;');
